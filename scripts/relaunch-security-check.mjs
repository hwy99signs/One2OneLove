...
check('community join cannot self-assign admin', communityMembership.includes("role = 'member'") && communityMembership.includes('ensure_community_creator_membership'), 'Creator admin must be server-created; self-join is member-only.');
...
check('frontend pricing matches approved launch price', membershipConfig.includes('introMonthly: 1.99') && membershipConfig.includes('introMonths: 6') && membershipConfig.includes('standardMonthly: 5.99'), 'The relaunch UI must not revive the retired tier prices.');
...
check('legacy membership gate is disabled pending entitlement approval', featureHook.includes('VITE_MEMBERSHIP_GATING_ENABLED') && featureHook.includes('approvedFeatureMapExists = false'), 'Do not invent a free-vs-paid entitlement boundary.');
