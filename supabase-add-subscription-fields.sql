...
-- First, ensure all 'Basis' plans are updated to 'Basic'. Run this statement separately before running the rest of the script.
-- UPDATE public.users SET subscription_plan = 'Basic' WHERE subscription_plan = 'Basis';
...
DO $$
BEGIN
  -- First, update any existing 'Basis' values to 'Basic'
  EXECUTE 'UPDATE public.users SET subscription_plan = ''Basic'' WHERE subscription_plan = ''Basis'';';

  -- Rename existing check constraint for subscription plan if it exists
  EXECUTE 'ALTER TABLE public.users RENAME CONSTRAINT check_subscription_plan TO check_subscription_plan_old;';
EXCEPTION
