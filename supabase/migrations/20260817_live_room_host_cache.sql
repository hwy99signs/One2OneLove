...
-- service_role to atomically claim one generation slot per room/time bucket so many
-- members entering the same quiet room do not create duplicate OpenAI calls.
...
comment on table public.live_room_host_prompt_cache is
  'Server-only AI Host cache/cost guard. One generation claim per live-room time bucket; browser roles have no direct access.';
...
-- 3. Confirm simultaneous requests produce at most one AI generation per room bucket.
-- 4. Add a periodic retention cleanup later if prompt volume makes it necessary.
