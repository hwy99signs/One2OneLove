...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api/diagnostics/stripe-secret-kind') {
      const key = String(env.STRIPE_SECRET_KEY || '');
      const kind = key.startsWith('rk_live_') ? 'restricted_live'
        : key.startsWith('sk_live_') ? 'secret_live'
        : key.startsWith('mk_') ? 'managed_key_id'
        : key ? 'other' : 'missing';
