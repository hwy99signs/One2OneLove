...
function AccountActionModal({ member, onClose, onComplete }) {
  const [action,setAction] = useState(member?.adminAction || 'activate');
...
export default function Admin() {
  ...
  const nav = (
    <>
      ...
      <div className="mt-auto border-t border-slate-200 p-4"><div className="mb-3 rounded-xl bg-blue-50 p-3 text-xs leading-5 text-blue-800"><ShieldCheck size={16} className="mb-1"/>Secure admin-only access. Member account controls are enabled and every change requires a recorded reason.</div><button onClick={()=>navigate('/Home')} className="flex w-full items-center gap-2 rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50"><ArrowLeft size={16}/>Exit Admin</button></div>
    </>
  );
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <main className="lg:pl-64">
        ...
        <div className="px-4 py-6 sm:px-6 lg:px-8">
          ...
          {section==='members' && <div>
            <Heading title="Members" subtitle="Activate, deactivate, suspend, or change a member's tier. Every action requires a reason before it can be confirmed."/>
            <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex max-w-md flex-1 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 shadow-sm">
              ...
              <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-2 text-sm font-bold text-emerald-800">Account Controls Enabled</div>
            </div>

            <div className="grid gap-4 xl:grid-cols-2">
              {filteredMembers.map(m => {
                const accountStatus = m.banned ? 'Suspended' : m.is_active === false ? 'Deactivated' : 'Active';
                const protectedAdmin = m.auth_role === 'admin';
                const openAction = action => setManagedMember({ ...m, adminAction: action });
                return (
                  <div key={m.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                      <div className="min-w-0">
                        <div className="truncate text-lg font-black text-slate-900">{m.name || 'Unnamed member'}</div>
                        <div className="truncate text-sm text-slate-500">{m.email}</div>
                        {m.location && <div className="mt-1 text-xs text-slate-400">{m.location}</div>}
                        <div className="mt-3 flex flex-wrap gap-2">
                          <Pill tone="blue">{m.subscription_plan || 'Basic'}</Pill>
                          <Pill tone={accountStatus === 'Active' ? 'green' : 'red'}>{accountStatus}</Pill>
                          <Pill tone="slate">{m.user_type || 'user'}</Pill>
                        </div>
                        {m.ban_reason && <div className="mt-2 rounded-lg bg-rose-50 p-2 text-xs text-rose-700"><strong>Suspension reason:</strong> {m.ban_reason}</div>}
                        <div className="mt-2 text-xs text-slate-400">Joined {date(m.created_at)}</div>
                      </div>

                      {protectedAdmin && <Pill tone="purple">Admin Protected</Pill>}
                    </div>

                    <div className="mt-5 border-t border-slate-100 pt-4">
                      <div className="mb-2 text-xs font-black uppercase tracking-[0.12em] text-slate-400">Account Controls</div>
                      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                        <button type="button" disabled={protectedAdmin} onClick={()=>openAction('activate')} className="rounded-xl border border-emerald-300 bg-emerald-50 px-3 py-2.5 text-xs font-black text-emerald-800 hover:bg-emerald-100 disabled:cursor-not-allowed disabled:opacity-40">ACTIVATE</button>
                        <button type="button" disabled={protectedAdmin} onClick={()=>openAction('deactivate')} className="rounded-xl border border-slate-300 bg-slate-50 px-3 py-2.5 text-xs font-black text-slate-800 hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-40">DEACTIVATE</button>
                        <button type="button" disabled={protectedAdmin} onClick={()=>openAction('suspend')} className="rounded-xl border border-rose-300 bg-rose-50 px-3 py-2.5 text-xs font-black text-rose-800 hover:bg-rose-100 disabled:cursor-not-allowed disabled:opacity-40">SUSPEND</button>
                        <button type="button" disabled={protectedAdmin} onClick={()=>openAction('change_tier')} className="rounded-xl border border-violet-300 bg-violet-50 px-3 py-2.5 text-xs font-black text-violet-800 hover:bg-violet-100 disabled:cursor-not-allowed disabled:opacity-40">CHANGE TIER</button>
                      </div>
                    </div>
                  </div>
                );
            ...
            {!filteredMembers.length && <Empty>No members match your search.</Empty>}
          </div>}
