...
import { ArrowRight, Brain, FileText, Heart, MessageCircle, Mic, Search, Target, X } from 'lucide-react';
...
const copy = {
  en: {
    title:'Relationship Support', subtitle:'Practical tools and learning resources to help relationships grow stronger.', search:'Search support resources…', all:'All', interactive:'Interactive Tools', learning:'Learning Content', explore:'Explore',
    goals:['Relationship Goals','Set meaningful goals and track progress together.'], quizzes:['Relationship Quizzes','Use self-reflection quizzes to better understand communication, conflict and relationship patterns.'], communication:['Communication Practice','Practice healthier responses in realistic relationship scenarios.'], articles:['Relationship Library','Read practical One2OneLove guides on communication, trust, dating, marriage and repair.'], podcasts:['Podcasts','Explore relationship podcasts and conversations across key relationship topics.'],
  ...
  fr: {
    title:'Soutien aux Relations', subtitle:'Des outils pratiques et des ressources pour aider les relations à se renforcer.', search:'Rechercher des ressources…', all:'Tous', interactive:'Outils Interactifs', learning:'Contenu d’Apprentissage', explore:'Explorer',
    goals:['Objectifs de Relation','Fixez des objectifs significatifs et suivez vos progrès ensemble.'], quizzes:['Quiz Relationnels','Utilisez des quiz d’auto-réflexion pour mieux comprendre communication, conflit et schémas relationnels.'], communication:['Pratique de Communication','Entraînez-vous à des réponses plus saines dans des scénarios réalistes.'], articles:['Bibliothèque des Relations','Lisez des guides pratiques One2OneLove sur la communication, la confiance, les rencontres, le mariage et la réparation.'], podcasts:['Podcasts','Explorez des podcasts et conversations sur les grands sujets relationnels.'],
  ...
  it: {
    title:'Supporto per le Relazioni', subtitle:'Strumenti pratici e risorse per aiutare le relazioni a diventare più forti.', search:'Cerca risorse di supporto…', all:'Tutti', interactive:'Strumenti Interattivi', learning:'Contenuti di Apprendimento', explore:'Esplora',
    goals:['Obiettivi di Relazione','Stabilite obiettivi significativi e seguite insieme i progressi.'], quizzes:['Quiz sulle Relazioni','Usa quiz di autoriflessione per comprendere meglio comunicazione, conflitto e schemi relazionali.'], communication:['Pratica di Comunicazione','Esercitati con risposte più sane in scenari realistici.'], articles:['Biblioteca delle Relazioni','Leggi guide pratiche One2OneLove su comunicazione, fiducia, incontri, matrimonio e riparazione.'], podcasts:['Podcasts','Esplora podcast e conversazioni sui principali temi relazionali.'],
    ...
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-purple-50 to-blue-50 px-4 py-12">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mx-auto mb-8 max-w-3xl rounded-3xl border border-purple-100 bg-white p-5 shadow-sm">
          <div className="relative"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={19}/><input value={search} onChange={event => setSearch(event.target.value)} placeholder={t.search} className="h-12 w-full rounded-2xl border border-slate-200 pl-11 pr-11 outline-none focus:border-purple-400 focus:ring-4 focus:ring-purple-100"/>{search && <button type="button" onClick={() => setSearch('')} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400"><X size={18}/></button>}</div>
