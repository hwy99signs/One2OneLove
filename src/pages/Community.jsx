      ...
      <div className="mx-auto max-w-6xl">
        ...
        {activeTab === 'stories' ? (
          <section className="mt-8" aria-labelledby="community-stories-heading">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                ...
                {user ? <Button onClick={() => setShowStoryForm(true)}>{t.share}</Button> : <Button asChild variant="outline"><Link to="/SignIn">{t.signIn.split('.')[0]}</Link></Button>}
              </div>
            </div>
          ...
          <section className="mt-8" aria-labelledby="community-buddies-heading">
            ...
            {!user ? <Card className="mt-6"><CardContent className="p-8 text-center"><p className="text-slate-600">{t.signIn}</p><Button asChild className="mt-4"><Link to="/SignIn">Sign In</Link></Button></CardContent></Card> : <div className="mt-7 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{!buddiesLoading && buddies.length === 0 && <p className="md:col-span-2 lg:col-span-3 rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center text-slate-500">{t.noBuddies}</p>}{buddies.map((buddy) => <BuddyCard key={buddy.request_id || buddy.id} buddy={buddy} />)}</div>}
          </section>
        )}
