        ...
        <section className="mt-10">
          ...
          <div className="mt-5 rounded-3xl border border-purple-100 bg-white p-5 shadow-sm">
            <div className="grid gap-4 md:grid-cols-[1.5fr_1fr_1fr]">
              <label className="text-sm font-bold text-slate-700">{t.searchLabel}<div className="relative mt-1"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder={t.search} className="h-12 w-full rounded-2xl border border-slate-200 pl-11 pr-10 font-medium text-slate-700 outline-none focus:border-purple-400 focus:ring-4 focus:ring-purple-100"/>{search&&<button type="button" onClick={()=>setSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400"><X size={17}/></button>}</div></label>
