...
const bookingWindow = (dateString, timeString, durationMinutes) => {
  ...
  const ends = new Date(starts.getTime() + Number(durationMinutes) * 60 * 1000);
  return { starts, ends };
};
...
export default function CreatorProgramming() {
  ...
  const selectedSlotConflict = useMemo(() => {
    ...
    return schedule.some((slot) => overlaps(slot, selectedWindow.starts, selectedWindow.ends));
  }, [schedule, selectedWindow]);

  const freeSlotsUsed = useMemo(
    () => mine.filter((slot) => slot.booking_tier === 'free' && slot.status === 'booked' && slot.creator_local_date === date).length,
    [mine, date]
  );

  const submit = async (event) => {
    event.preventDefault();
    if (!title.trim() || !startTime || !policyAcknowledged || selectedSlotConflict || freeSlotsUsed >= 2 || !selectedWindow) return;
    setBooking(true);
    try {
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-900">
      <section className="bg-gradient-to-br from-violet-700 via-fuchsia-600 to-pink-600 px-5 py-14 text-white"><div className="mx-auto max-w-7xl"><div className="inline-flex items-center gap-2 rounded-full bg-white/15 px-4 py-2 text-xs font-black tracking-[0.16em]"><Radio className="h-4 w-4" />{t.badge}</div><h1 className="mt-5 max-w-4xl text-4xl font-black tracking-tight sm:text-5xl">{t.title}</h1><p className="mt-5 max-w-3xl text-lg leading-8 text-white/90">{t.intro}</p><div className="mt-7 flex flex-wrap gap-3"><span className="rounded-full bg-white px-4 py-2 text-sm font-black text-violet-700">{t.freeRule}</span><span className="rounded-full border border-white/30 bg-white/10 px-4 py-2 text-sm font-bold">{t.paidRule}</span></div></div></section>
      ...
      <section className="mx-auto max-w-7xl px-5 py-10">
        ...
        <div className="grid gap-8 lg:grid-cols-[1.25fr_.75fr]">
          <div>
            ...
            <div className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm"><div className="flex items-center gap-2 border-b bg-slate-950 px-5 py-4 font-black text-white"><Clock3 className="h-5 w-5" />{t.schedule}</div>{HOURS.map((hour) => { const slots = scheduleByHour.get(hour) || []; return <div key={hour} className="grid min-h-16 grid-cols-[90px_1fr] border-b border-slate-100 last:border-b-0"><div className="border-r bg-slate-50 px-3 py-4 text-sm font-black text-slate-500">{timeLabel(hour)}</div><div className="p-2">{slots.length ? slots.map((slot) => <div key={`${hour}-${slot.id}`} className={`rounded-xl border px-3 py-2 ${slot.program_source === 'o2ol' ? 'border-fuchsia-200 bg-fuchsia-50' : 'border-violet-200 bg-violet-50'}`}><div className="flex flex-wrap items-center justify-between gap-2"><span className="font-black text-slate-950">{slot.title}</span><div className="flex gap-1.5"><span className="rounded-full bg-white px-2 py-0.5 text-[10px] font-black uppercase text-slate-700">{slot.program_source === 'o2ol' ? t.sourceO2OL : t.sourceCreator}</span><span className="rounded-full bg-violet-700 px-2 py-0.5 text-[10px] font-black uppercase text-white">{slot.content_mode === 'replay' ? t.replay : t.live}</span></div></div><div className="mt-1 text-xs font-semibold text-slate-600">{slotTimeLabel(slot)} · {t.booked}</div></div>) : <div className="px-2 py-3 text-sm font-semibold text-emerald-700">{t.open}</div>}</div></div>; })}</div>
          </div>

          <div className="space-y-6">
            <form onSubmit={submit} className="rounded-[2rem] border border-violet-100 bg-white p-6 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-2 text-xl font-black"><CalendarDays className="h-5 w-5 text-violet-600" />{t.bookTitle}</div><span className={`rounded-full px-3 py-1 text-xs font-black ${freeSlotsUsed >= 2 ? 'bg-rose-100 text-rose-800' : 'bg-emerald-100 text-emerald-800'}`}>{freeSlotsUsed}/2 {t.freeUsed}</span></div><div className="mt-5 space-y-4"><div><label className="mb-1.5 block text-sm font-bold">{t.programTitle}</label><Input value={title} onChange={(event) => setTitle(event.target.value.slice(0, 120))} required /></div><div><label className="mb-1.5 block text-sm font-bold">{t.description}</label><textarea value={description} onChange={(event) => setDescription(event.target.value.slice(0, 1000))} rows={3} className="w-full rounded-md border border-slate-200 px-3 py-2 text-sm outline-none focus:border-violet-400" /></div><div className="grid grid-cols-2 gap-3"><div><label className="mb-1.5 block text-sm font-bold">{t.start}</label><Input type="time" value={startTime} onChange={(event) => setStartTime(event.target.value)} required /></div><div><label className="mb-1.5 block text-sm font-bold">{t.duration}</label><select value={duration} onChange={(event) => setDuration(Number(event.target.value))} className="h-10 w-full rounded-md border border-slate-200 bg-white px-3 text-sm">{DURATIONS.map((value) => <option key={value} value={value}>{value} {t.minutes}</option>)}</select></div></div>{selectedSlotConflict ? <div className="rounded-2xl border border-rose-200 bg-rose-50 p-3 text-sm font-bold leading-6 text-rose-800">{t.conflictWarning}</div> : null}<div><label className="mb-1.5 block text-sm font-bold">{t.mode}</label><div className="grid grid-cols-2 gap-2"><button type="button" onClick={() => setMode('live')} className={`rounded-xl border p-3 text-sm font-black ${mode === 'live' ? 'border-violet-500 bg-violet-50 text-violet-800' : 'border-slate-200'}`}><Radio className="mx-auto mb-1 h-4 w-4" />{t.live}</button><button type="button" onClick={() => setMode('replay')} className={`rounded-xl border p-3 text-sm font-black ${mode === 'replay' ? 'border-violet-500 bg-violet-50 text-violet-800' : 'border-slate-200'}`}><Video className="mx-auto mb-1 h-4 w-4" />{t.replay}</button></div></div>{mode === 'replay' && <div><label className="mb-1.5 block text-sm font-bold">{t.replayUrl}</label><Input type="url" value={replayUrl} onChange={(event) => setReplayUrl(event.target.value.slice(0, 1000))} required /></div>}<label className="flex items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm leading-6 text-slate-700"><input type="checkbox" checked={policyAcknowledged} onChange={(event) => setPolicyAcknowledged(event.target.checked)} className="mt-1 h-4 w-4 rounded border-slate-300" required /><span>{t.policyLabel}</span></label><Button type="submit" disabled={booking || !policyAcknowledged || selectedSlotConflict || freeSlotsUsed >= 2} className="w-full bg-violet-700 text-white hover:bg-violet-800">{booking ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}{booking ? t.booking : t.book}</Button><div className="flex items-center justify-between text-xs font-bold text-slate-500"><span>{t.free}</span><span>{t.paidLater}</span></div></div></form>
