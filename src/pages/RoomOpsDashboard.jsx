...
import { Activity, AlertTriangle, CalendarClock, CalendarX2, Flag, Radio, RotateCcw, ShieldCheck, Users } from 'lucide-react';
...
const translations = {
  en: { title: 'Global Relationship Room Operations', subtitle: 'A private operational snapshot for trusted One2OneLove room moderators.', back: 'Back to the Global Relationship Room', restricted: 'Moderator access required', restrictedCopy: 'This operations dashboard is limited to trusted Global Relationship Room moderators.', loading: 'Loading room operations…', loadError: 'Room operations data is temporarily unavailable. Please try again.', pendingCreators: 'Creator applications', pendingPrograms: 'Programs awaiting review', reports: 'Open viewer reports', cancellations: 'Open cancellation requests', live: 'Programs live now', next24: 'Programs in next 24 hours', next7: 'Programs in next 7 days', creators: 'Approved creators', moderation: 'Moderation Queue', programManager: 'Program Manager', reportsQueue: 'Viewer Reports', cancellationQueue: 'Cancellation Requests', official: 'Official Programming', replays: 'Replay Manager', audit: 'Moderation History' },
  es: { title: 'Operaciones de la Sala Global de Relaciones', subtitle: 'Resumen operativo privado para moderadores de confianza de One2OneLove.', back: 'Volver a la Sala Global de Relaciones', restricted: 'Se requiere acceso de moderador', restrictedCopy: 'Este panel está limitado a moderadores de confianza de la Sala Global.', loading: 'Cargando operaciones…', loadError: 'Los datos operativos no están disponibles temporalmente. Inténtalo de nuevo.', pendingCreators: 'Solicitudes de creadores', pendingPrograms: 'Programas pendientes de revisión', reports: 'Reportes abiertos de espectadores', cancellations: 'Solicitudes de cancelación abiertas', live: 'Programas en vivo ahora', next24: 'Programas en próximas 24 horas', next7: 'Programas en próximos 7 días', creators: 'Creadores aprobados', moderation: 'Cola de Moderación', programManager: 'Administrador de Programas', reportsQueue: 'Reportes de Espectadores', cancellationQueue: 'Solicitudes de Cancelación', official: 'Programación Oficial', replays: 'Administrador de Repeticiones', audit: 'Historial de Moderación' },
  fr: { title: 'Opérations de la Salle Mondiale des Relations', subtitle: 'Aperçu opérationnel privé pour les modérateurs de confiance One2OneLove.', back: 'Retour à la Salle Mondiale des Relations', restricted: 'Accès modérateur requis', restrictedCopy: 'Ce tableau de bord est réservé aux modérateurs de confiance de la Salle Mondiale.', loading: 'Chargement des opérations…', loadError: 'Les données opérationnelles sont temporairement indisponibles. Veuillez réessayer.', pendingCreators: 'Candidatures créateurs', pendingPrograms: 'Programmes en attente', reports: 'Signalements ouverts', cancellations: 'Demandes d’annulation ouvertes', live: 'Programmes en direct', next24: 'Programmes dans les 24 prochaines heures', next7: 'Programmes dans les 7 prochains jours', creators: 'Créateurs approuvés', moderation: 'File de Modération', programManager: 'Gestionnaire des Programmes', reportsQueue: 'Signalements Spectateurs', cancellationQueue: 'Demandes d’Annulation', official: 'Programmation Officielle', replays: 'Gestionnaire des Rediffusions', audit: 'Historique de Modération' },
  it: { title: 'Operazioni della Sala Globale delle Relazioni', subtitle: 'Panoramica operativa privata per i moderatori fidati One2OneLove.', back: 'Torna alla Sala Globale delle Relazioni', restricted: 'Accesso moderatore richiesto', restrictedCopy: 'Questo pannello è riservato ai moderatori fidati della Sala Globale.', loading: 'Caricamento operazioni…', loadError: 'I dati operativi non sono temporaneamente disponibili. Riprova.', pendingCreators: 'Richieste creator', pendingPrograms: 'Programmi in attesa di revisione', reports: 'Segnalazioni aperte', cancellations: 'Richieste di cancellazione aperte', live: 'Programmi in diretta', next24: 'Programmi nelle prossime 24 ore', next7: 'Programmi nei prossimi 7 giorni', creators: 'Creator approvati', moderation: 'Coda Moderazione', programManager: 'Gestione Programmi', reportsQueue: 'Segnalazioni Spettatori', cancellationQueue: 'Richieste di Cancellazione', official: 'Programmazione Ufficiale', replays: 'Gestione Repliche', audit: 'Cronologia Moderazione' },
  de: { title: 'Betrieb des Globalen Beziehungsraums', subtitle: 'Private Betriebsübersicht für vertrauenswürdige One2OneLove-Moderatoren.', back: 'Zurück zum Globalen Beziehungsraum', restricted: 'Moderatorzugang erforderlich', restrictedCopy: 'Dieses Dashboard ist auf vertrauenswürdige Moderatoren des Globalen Beziehungsraums beschränkt.', loading: 'Raumbetrieb wird geladen…', loadError: 'Betriebsdaten sind vorübergehend nicht verfügbar. Bitte versuche es erneut.', pendingCreators: 'Creator-Bewerbungen', pendingPrograms: 'Programme in Prüfung', reports: 'Offene Zuschauer-Meldungen', cancellations: 'Offene Stornierungsanfragen', live: 'Jetzt laufende Programme', next24: 'Programme in den nächsten 24 Stunden', next7: 'Programme in den nächsten 7 Tagen', creators: 'Freigegebene Creator', moderation: 'Moderationsliste', programManager: 'Programmverwaltung', reportsQueue: 'Zuschauer-Meldungen', cancellationQueue: 'Stornierungsanfragen', official: 'Offizielle Programme', replays: 'Wiederholungsverwaltung', audit: 'Moderationsverlauf' },
};
...
export default function RoomOpsDashboard() {
  ...
  const links = [
    ...
    ['/RoomCancellationQueue', t.cancellationQueue, CalendarX2],
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-rose-50 px-4 py-10 md:py-14">
      <div className="mx-auto max-w-6xl">
        ...
        {summaryQuery.isLoading ? <div role="status" aria-live="polite" className="mt-6 rounded-2xl bg-slate-50 p-6 text-center text-slate-500">{t.loading}</div> : summaryFailed || !s ? (
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Metric icon={Users} label={t.pendingCreators} value={s.pendingCreators} /><Metric icon={ShieldCheck} label={t.pendingPrograms} value={s.pendingPrograms} /><Metric icon={Flag} label={t.reports} value={s.openReports} /><Metric icon={CalendarX2} label={t.cancellations} value={s.openCancellations} /><Metric icon={Radio} label={t.live} value={s.liveNow} /><Metric icon={CalendarClock} label={t.next24} value={s.next24Hours} /><Metric icon={CalendarClock} label={t.next7} value={s.next7Days} /><Metric icon={Users} label={t.creators} value={s.approvedCreators} /></div>
        )}
