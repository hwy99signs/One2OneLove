...
function BrowserVisual({ question, index }) {
  const { presentation, theme, sceneUrl } = getQuestionVisual(question, index);

  if (presentation === 'scene') {
    return (
      <div className="relative h-28 w-full overflow-hidden rounded-xl bg-slate-200 sm:h-32 sm:w-44 sm:shrink-0">
        <img src={sceneUrl} alt="" className="absolute inset-0 h-full w-full object-cover" onError={(event) => { event.currentTarget.style.display = 'none'; }} />
        <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent" />
        <div className="absolute bottom-2 left-2 rounded-full bg-white/90 px-2.5 py-1 text-[9px] font-black uppercase text-[#102f60]">{theme.label}</div>
      </div>
    );
  }

  return (
    <div className="relative flex h-28 w-full items-center justify-center overflow-hidden rounded-xl sm:h-32 sm:w-44 sm:shrink-0" style={{ background: theme.gradient }}>
      <div className="absolute -right-6 -top-8 h-24 w-24 rounded-full bg-white/35" />
      <div className="opacity-30" style={{ color: theme.accent }}><CategoryIcon iconKey={theme.iconKey} className="h-14 w-14" /></div>
      <div className="absolute bottom-2 left-2 rounded-full bg-white/85 px-2.5 py-1 text-[9px] font-black uppercase text-[#102f60]">{theme.label}</div>
    </div>
  );
}
        ...
        <div className="sticky top-0 z-10 border-b border-slate-200 bg-white px-5 py-5 sm:px-8 sm:py-7">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h2 className="mt-1 text-2xl font-black leading-tight text-[#102f60] sm:text-4xl">Browse What Should They Do?</h2>
              <p className="mt-2 text-sm font-semibold text-slate-500 sm:text-base">{filtered.length} of {questions.length} questions · {categories.length} categories</p>
            </div>
          ...
          <label className="mt-5 block">
            ...
            <div className="relative flex min-h-14 items-center rounded-2xl border-2 border-slate-200 bg-slate-50 transition focus-within:border-[#1976f3]">
              <Search className="pointer-events-none absolute left-4 h-5 w-5 text-[#56708f]" />
              <select
                value={category}
                onChange={(event) => setCategory(event.target.value)}
                className="h-14 w-full appearance-none bg-transparent pl-12 pr-12 text-base font-bold text-[#17345f] outline-none sm:text-lg"
              >
                <option value="all">Choose a Category — All Categories ({questions.length})</option>
                {categories.map(item => {
                  const count = questions.filter(question => question.category === item).length;
                  return <option key={item} value={item}>{getQuestionTheme(item).label} ({count})</option>;
                })}
              </select>
          ...
          {filtered.map((item) => {
            ...
            return (
              <button
                ...
                className="group flex w-full flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-4 text-left transition hover:border-[#ff176b] hover:shadow-md sm:flex-row sm:items-center sm:p-5"
              >
                <BrowserVisual question={item} index={index} />
                <div className="min-w-0 flex-1">
                  <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                    <span className="inline-flex items-center gap-1.5 rounded-full bg-blue-50 px-3 py-1.5 text-[10px] font-black uppercase tracking-wide text-[#1768c4] sm:text-xs">
                  ...
                  <span className="mt-4 inline-flex items-center gap-1.5 text-sm font-black text-[#ff176b] sm:text-base">Open question <ChevronRight className="h-4 w-4 transition group-hover:translate-x-0.5" /></span>
                </div>
              </button>
            );
          })}
...
export default function WhatShouldTheyDoGameHybridReadable({ onExit }) {
