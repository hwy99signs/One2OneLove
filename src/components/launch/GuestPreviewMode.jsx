...
import { getGuestPreview, guestPreviewRemainingLabel, endGuestPreview } from '@/lib/guestPreview';
...
export default function GuestPreviewMode() {
  ...
  const [preview, setPreview] = useState(() => getGuestPreview());

  useEffect(() => {
    const refresh = () => setPreview(getGuestPreview());
    refresh();
    ...
    <div className="sticky top-0 z-[65] border-b border-amber-300 bg-amber-50 px-3 py-2 text-amber-950 shadow-sm" data-guest-preview-allow="true">
      <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-center gap-x-3 gap-y-2 text-center text-sm sm:justify-between sm:text-left">
        ...
        <div className="flex items-center gap-2">
          <span className="rounded-full bg-white px-3 py-1 text-xs font-bold shadow-sm">{guestPreviewRemainingLabel()}</span>
          ...
          <button type="button" onClick={() => { endGuestPreview(); navigate('/SignIn'); }} className="rounded-lg border border-amber-400 bg-white px-3 py-1.5 text-xs font-bold hover:bg-amber-100">End Preview</button>
        </div>
      </div>
    </div>
  );
