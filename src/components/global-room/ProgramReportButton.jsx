...
export default function ProgramReportButton({ slotId, compact = false }) {
  ...
  if (!isAuthenticated) {
    return compact ? null : <span className="text-xs text-slate-400">{t.signIn}</span>;
  }

  return (
    <>
      <button type="button" onClick={() => { setOpen(true); setMessage(''); setIsError(false); }} className="inline-flex items-center gap-1 text-xs font-medium text-slate-400 hover:text-rose-700">
        <Flag className="h-3.5 w-3.5" /> {t.report}
      </button>

      {open && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/45 p-4" role="dialog" aria-modal="true" aria-labelledby={`report-program-${slotId}`}>
          <div className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-4">
              ...
              <button type="button" onClick={() => setOpen(false)} className="rounded-full p-2 text-slate-400 hover:bg-slate-100" aria-label={t.cancel}><X className="h-5 w-5" /></button>
            </div>

            {message && <div className={`mt-4 rounded-xl border p-3 text-sm ${isError ? 'border-red-200 bg-red-50 text-red-800' : 'border-emerald-200 bg-emerald-50 text-emerald-800'}`}>{message}</div>}
