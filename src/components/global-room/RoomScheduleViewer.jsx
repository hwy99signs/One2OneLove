...
const translations = {
  en: {
    schedule: 'Programming Schedule', liveNow: 'Live Now', upNext: 'Up Next', sevenDays: 'Next 7 Days', loading: 'Loading programming…', empty: 'No public programming is scheduled in this window yet.', nothingLive: 'No program is live right now.', noUpcoming: 'No additional programs are scheduled yet.', live: 'LIVE', official: 'One2OneLove', creator: 'Creator', replay: 'Replay', partner: 'Partner', special: 'Special', presentedBy: 'Presented by', thirdParty: 'Third-party views are not necessarily those of One2OneLove or ERANT.'
  },
  es: {
    schedule: 'Programación', liveNow: 'En Vivo Ahora', upNext: 'A Continuación', sevenDays: 'Próximos 7 Días', loading: 'Cargando programación…', empty: 'Todavía no hay programación pública en este período.', nothingLive: 'No hay ningún programa en vivo en este momento.', noUpcoming: 'Todavía no hay programas adicionales programados.', live: 'EN VIVO', official: 'One2OneLove', creator: 'Creador', replay: 'Repetición', partner: 'Socio', special: 'Especial', presentedBy: 'Presentado por', thirdParty: 'Las opiniones de terceros no representan necesariamente a One2OneLove o ERANT.'
  },
  fr: {
    schedule: 'Programme', liveNow: 'En Direct', upNext: 'À Suivre', sevenDays: '7 Prochains Jours', loading: 'Chargement du programme…', empty: 'Aucun programme public n’est encore prévu dans cette période.', nothingLive: 'Aucun programme n’est en direct pour le moment.', noUpcoming: 'Aucun autre programme n’est encore prévu.', live: 'EN DIRECT', official: 'One2OneLove', creator: 'Créateur', replay: 'Rediffusion', partner: 'Partenaire', special: 'Spécial', presentedBy: 'Présenté par', thirdParty: 'Les opinions de tiers ne représentent pas nécessairement One2OneLove ou ERANT.'
  },
  it: {
    schedule: 'Programmazione', liveNow: 'In Diretta Ora', upNext: 'A Seguire', sevenDays: 'Prossimi 7 Giorni', loading: 'Caricamento della programmazione…', empty: 'Non ci sono ancora programmi pubblici previsti in questo intervallo.', nothingLive: 'Nessun programma è in diretta in questo momento.', noUpcoming: 'Non ci sono ancora altri programmi in calendario.', live: 'IN DIRETTA', official: 'One2OneLove', creator: 'Creator', replay: 'Replica', partner: 'Partner', special: 'Speciale', presentedBy: 'Presentato da', thirdParty: 'Le opinioni di terzi non rappresentano necessariamente One2OneLove o ERANT.'
  },
  de: {
    schedule: 'Programmplan', liveNow: 'Jetzt Live', upNext: 'Als Nächstes', sevenDays: 'Nächste 7 Tage', loading: 'Programm wird geladen…', empty: 'Für diesen Zeitraum ist noch kein öffentliches Programm geplant.', nothingLive: 'Derzeit läuft kein Programm live.', noUpcoming: 'Es sind noch keine weiteren Programme geplant.', live: 'LIVE', official: 'One2OneLove', creator: 'Creator', replay: 'Wiederholung', partner: 'Partner', special: 'Spezial', presentedBy: 'Präsentiert von', thirdParty: 'Ansichten Dritter entsprechen nicht notwendigerweise denen von One2OneLove oder ERANT.'
  },
};
...
function TypeBadge({ type, t }) {
  ...
  return <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700"><Icon className="h-3.5 w-3.5" /> {label}</span>;
}
  ...
  return (
    <div className={`rounded-2xl border p-4 ${live ? 'border-rose-200 bg-rose-50/70' : 'border-slate-200 bg-white'}`}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="shrink-0 rounded-xl bg-slate-50 px-3 py-2 text-sm font-semibold text-slate-700"><Clock3 className="mr-1.5 inline h-4 w-4 text-rose-600" />{formatTime(slot.scheduled_start, language)}</div>
      </div>
    </div>
  );
}
...
export default function RoomScheduleViewer({ slots = [], isLoading = false }) {
  ...
  if (isLoading) {
    return <Card className="rounded-3xl border-slate-200"><CardHeader><CardTitle className="text-2xl">{t.schedule}</CardTitle></CardHeader><CardContent><p className="py-8 text-slate-500">{t.loading}</p></CardContent></Card>;
  }

  if (!slots.length) {
    return <Card className="rounded-3xl border-slate-200"><CardHeader><CardTitle className="text-2xl">{t.schedule}</CardTitle></CardHeader><CardContent><div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-8 text-center text-slate-500">{t.empty}</div></CardContent></Card>;
  }

  return (
    <div className="space-y-6">
      <Card className="rounded-3xl border-rose-100">
        <CardHeader><CardTitle className="flex items-center gap-2 text-2xl"><Radio className="h-5 w-5 text-rose-600" />{t.liveNow}</CardTitle></CardHeader>
        <CardContent>{liveSlots.length ? <div className="space-y-3">{liveSlots.map((slot) => <ProgramCard key={slot.id} slot={slot} t={t} language={currentLanguage} live />)}</div> : <p className="rounded-2xl bg-slate-50 p-5 text-sm text-slate-500">{t.nothingLive}</p>}</CardContent>
      </Card>
      ...
      <Card className="rounded-3xl border-slate-200">
        ...
        <CardContent><div className="space-y-7">{Object.entries(grouped).map(([key, daySlots]) => <section key={key}><h3 className="mb-3 text-sm font-bold uppercase tracking-wide text-slate-500">{formatDay(daySlots[0].scheduled_start, currentLanguage)}</h3><div className="space-y-3">{daySlots.map((slot) => <ProgramCard key={slot.id} slot={slot} t={t} language={currentLanguage} />)}</div></section>)}</div></CardContent>
      </Card>
    </div>
  );
}
