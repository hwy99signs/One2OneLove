...
const translations = {
  ...
  nl: { voiceCall: 'Spraakoproep', videoCall: 'Video-oproep', markAsUnread: 'Markeren als ongelezen', unpin: 'Losmaken', pin: 'Vastmaken', archive: 'Archiveren', delete: 'Verwijderen', noConversations: 'Nog geen gesprekken', noMatches: 'Geen overeenkomende gesprekken', startChat: 'Maak eerst verbinding met een lid en start daarna een privéchat', searchPlaceholder: 'Gesprekken zoeken' },
};
  ...
  return (
    <div className="flex h-full flex-col border-r border-gray-200 bg-white">
      <div className="border-b border-gray-200 p-4">
        <h2 className="mb-4 text-xl font-semibold text-gray-900">Chats</h2>
        ...
        {filteredConversations.length === 0 ? (
          ...
          <div key={chat.id} onClick={() => onSelectChat(chat.id)} className={cn('group flex cursor-pointer items-center gap-3 border-b border-gray-100 p-3 transition-colors hover:bg-gray-50', selectedChatId === chat.id && 'bg-gray-50')}>
            <div className="flex-shrink-0">
              <AvatarWithStatus userId={chat.otherUserId} avatarUrl={privacySafeAvatar(chat.avatar)} name={chat.name} size="lg" showStatus />
            </div>

            <div className="min-w-0 flex-1">
              <div className="mb-1 flex items-center justify-between"><h3 className="truncate text-sm font-semibold text-gray-900">{chat.name}</h3>{chat.lastMessageTime && <span className="ml-2 flex-shrink-0 text-xs text-gray-500">{formatLastSeen(chat.lastMessageTime)}</span>}</div>
              ...
              {chat.isMuted && <div className="mt-1 flex items-center gap-1"><span className="text-xs text-gray-400">🔇 Muted</span></div>}
            </div>

            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button type="button" aria-label={`Actions for ${chat.name || 'conversation'}`} onClick={(event) => event.stopPropagation()} className="flex-shrink-0 rounded-full p-1 opacity-0 transition-colors hover:bg-gray-200 group-hover:opacity-100 focus:opacity-100"><MoreVertical className="h-4 w-4 text-gray-600" /></button>
              </DropdownMenuTrigger>
